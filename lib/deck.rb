...
Squib::Deck.new(cards: 3) do
  ...
  text range: 1, font: 'Archivo Narrow,Sans 10', markup: true,
       spacing: -8,
  ...
  0.upto(6).each do |n|
    ...
    y -= 10 if n == 3 # Strong Magnet
    y -= 20 if n == 4 # Shiny Shiv
    y -= 40 if n == 5 # Tote bag adjustment
    y -= 70 if n == 6 # Whacking plank adjustment
    rect x: 75, y: y, width: 65, height: 65,
         radius: 15, range: 1, stroke_width: 4
  end
  text str: "<b>Zeppelin:</b>\n8 Metal, 6 Fabric, 4 Oil, 3 Duct Tape",
       font: 'Archivo Narrow,Sans 12', range: 1, markup: true,
       x: 75, y: 900, width: 675, ellipsize: false, align: :center
